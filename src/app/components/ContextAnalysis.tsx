...
export function ContextAnalysis(props: ContextAnalysisProps = {}) {
  ...
  const analysisType = data?.analysisType || 'complexity';
  const result = data?.result;
  const insights = data?.insights || [];

  const handleCreateHyperfocus = useCallback(async () => {
    if (window.openai?.sendFollowUpMessage && data?.contextDescription) {
      await window.openai.sendFollowUpMessage({
        prompt: `Criar hiperfoco: ${data.contextDescription}`,
      });
    }
