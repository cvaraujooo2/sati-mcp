...
export function SubtaskSuggestions() {
  ...
  const [selectedTasks, setSelectedTasks] = useState<Set<number>>(
    new Set(toolOutput?.suggestedTasks.map((_, index) => index) ?? [])
  );
  ...
  const complexity = toolOutput?.complexity || 'medium';
  const totalMinutes = toolOutput?.totalEstimatedMinutes || 0;
  ...
  const handleCreateTasks = useCallback(async () => {
    if (!toolOutput?.hyperfocusId || selectedTasks.size === 0) return;
  ...
  }, [toolOutput?.hyperfocusId, selectedTasks, suggestedTasks, hyperfocusTitle]);
